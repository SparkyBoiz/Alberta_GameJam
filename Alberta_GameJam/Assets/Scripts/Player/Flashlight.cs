...
public class Flashlight : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            // Try ChasingGhost
            var chasingGhost = other.GetComponentInParent<ChasingGhost>();
            if (chasingGhost != null)
            {
                ApplyDamageChasing(chasingGhost);
                return;
            }
            return;
        }
    ...
    void ApplyDamageChasing(ChasingGhost chasingGhost)
    {
        if (chasingGhost == null || damage <= 0)
        {
            return;
        }
        chasingGhost.TakeDamage(damage);
    }
